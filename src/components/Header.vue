...
<div class="fly-header layui-bg-black">
  <div class="layui-container">
    ...
    <ul class="layui-nav fly-nav-user">
        <li v-if="user.username" class="layui-nav-item">
          <a class="fly-nav-avatar" href="javascript:;">
            <cite class="layui-hide-xs">{{user.username}}</cite>
              <i v-if="user.approve" class="iconfont icon-renzheng layui-hide-xs" :title="`认证信息${user.approve}`"></i>
              <i v-if="user.rmb" class="layui-badge fly-badge-vip layui-hide-xs">VIP{{ user.rmb }}</i>
            <img :src="user.avatar">
          </a>
          <dl class="layui-nav-child">
            ...
            <dd><a :href="`/u/168*${user.id}`"><i class="layui-icon" style="margin-left: 2px; font-size: 22px;">&#xe68e;</i>我的主页</a></dd>
